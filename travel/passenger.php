	...
	<body>
        ...
        <script>
            ...
            document.getElementById("1").click(); }
                    ...
                    <div class="row animate-box">
                        <?php
                         echo "<h1 style='font-size: 30px'>".$airportID_start['fullName']."<i class='fas fa-arrows-alt-h'></i>".$airportID_destination['fullName']."</h1>";
...
if($booking)
{
    ...
    $sql_book_summary = "SELECT l.legID, a.fullname, b.fullname, d.airlineName, l.dateStart, l.dateEnd, l.duration, l.price 
        FROM legs AS l  
        INNER JOIN airport AS a
            ON l.airportIDStart=a.airportID
        INNER JOIN airport AS b
        ...
        INNER JOIN airline AS d
            ON c.airlineID=d.airlineID
        WHERE l.legID=$booking;";
    $query=mysqli_query($con, $sql_book_summary);
    ...
    //echo $number_ad."<br>";
    //echo $number_ch."<br>";
    if (!mysqli_query($con, $sql_book_summary)){   
        die('Error: ' . mysqli_error($con));
    }else {
         //passenger contact detail
        $main_form = "<form method ='POST' action = '/travel/payment.php'>";
        for($i=0; $i < $number_ad; $i++){
            $order = $i + 1;
            $main_form.="<h3 style='margin :0 0 -10px 0'>Adult $order</h3><br>
            First Name <input type='text' name='ad_fname[]' placeholder='Enter First Name' class='form-control name_list' />
            Last Name <input type='text' name='ad_lname[]' placeholder='Enter Last Name' class='form-control name_list' /><br>
            Sex <select name='ad_sex[]'><option value='Male' selected>Male</option><option value='Female'>Female</option></select><br><br>
            Special recommend <br><textarea rows='4' cols='50' placeholder='Enter special recommend' name='ad_comm[]'></textarea></p><br>";
        }
        for($i=0; $i < $number_ch; $i++){
            $order = $i + 1;
            $main_form.="<p>Child $order<br>
            First Name <input type='text' name='ch_fname[]' placeholder='Enter First Name' class='form-control name_list' />
            Last Name <input type='text' name='ch_lname[]' placeholder='Enter Last Name' class='form-control name_list' /><br>
            Sex <select name='ch_sex[]'><option value='Male' selected>Male</option><option value='Female'>Female</option></select><br><br>
            Special recommend <br><textarea rows='4' cols='50' placeholder='Enter special recommend' name='ch_comm[]'></textarea></p><br>";
        }
        $main_form .="<button type='submit' class='btn btn-primary btn-block' value= $search_round_trip name = 'Book'>Start Boonking</button><br>";
        $main_form .="</form>";
        echo $main_form;
    }

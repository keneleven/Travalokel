...
<?php
    ...
    $strQuery = "SELECT MONTHNAME(dateStart) as Month,COUNT(ci.email) as CustCount from customer_information ci
    INNER JOIN booking b ON ci.email = b.customerID
    INNER JOIN legs l ON b.legID = l.legID
    GROUP BY MONTHNAME(dateStart)
    ORDER BY COUNT(ci.email) DESC
     LIMIT 10";
    ...
    if ($result) {
        ...
        $arrData = array(
            "chart" => array(
              "caption" => "Number of customer in each month",
                        "paletteColors" => "#C85A17,#6C2DC7,#008080,#9DC209",
               "bgColor" => "#ffffff",
              "borderAlpha"=> "20",
              "canvasBorderAlpha"=> "0",

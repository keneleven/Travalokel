    ...
    $search_children = $_SESSION["children"];
            ...
            if($_SESSION["adult"] > 0){
                for($i = 0; $i < $_SESSION["adult"]; $i++){
                    if(trim($_POST["ad_fname"][$i] != '') && trim($_POST["ad_lname"][$i] != '') && trim($_POST["ad_sex"][$i] != '')){
                        $_SESSION["ad_fname"][$i] = mysqli_real_escape_string($con, $_POST["ad_fname"][$i]);
                        $_SESSION["ad_lname"][$i] = mysqli_real_escape_string($con, $_POST['ad_lname'][$i]);
                        $_SESSION["ad_sex"][$i] = mysqli_real_escape_string($con, $_POST['ad_sex'][$i]);
                        $_SESSION["ad_comm"][$i] = mysqli_real_escape_string($con, $_POST['ad_comm'][$i]);   
                        //var_dump($_SESSION["ad_fname"][$i]);
                    }
                }
                //var_dump($_SESSION["ad_fname"]);
            }
            if($_SESSION["children"] > 0){
                for($i = 0; $i < $_SESSION["children"]; $i++){
                    if(trim($_POST["ch_fname"][$i] != '') && trim($_POST["ch_lname"][$i] != '') && trim($_POST["ch_sex"][$i] != '')){
                        $_SESSION["ch_fname"][$i] = mysqli_real_escape_string($con, $_POST["ch_fname"][$i]);
                        $_SESSION["ch_lname"][$i] = mysqli_real_escape_string($con, $_POST['ch_lname'][$i]);
                        $_SESSION["ch_sex"][$i] = mysqli_real_escape_string($con, $_POST['ch_sex'][$i]);
                        $_SESSION["ch_comm"][$i] = mysqli_real_escape_string($con, $_POST['ch_comm'][$i]);
                    }
                }
            }
...
<div class="col-md-8 animate-box fadeInUp animated">
    ...
    <table id="t01">
            <form method='POST' action='/travel/confirm.php'>
                <tr><td>Payment Medthod:<br><select name="card_flag" class="form-control">
				<option value="0" selected>Credit Card</option>
				<option value="1">Transfer</option>
                </select></td><td colspan="5"></td></tr>
                <tr><td>Card Number:</td><td colspan="5"></td></tr><tr><td>
                    <input type='text' name='card_number' class="form-control" required></td><td colspan="5"></td></tr>
                <tr><td>Card Type:</td><td colspan="5"></td></tr>
                <tr><td><select name="card_type" class="form-control">
				<option value="Debit" selected>Debit</option>
				<option value="Credit">Credit</option>
                </select></td><td colspan="5"></td></tr>
                <tr><td>Card Holder:</td><td colspan="5"></td></tr>
                <tr><td colspan="3"><input type='text' name='card_holder'></td><td colspan="3"></td></tr>
                <tr><td>CV:<input type='text' class="form-control" name='cv'></td><td colspan="4"></td>
                <td>Validity Date:<input type='text'  class="form-control" name='validity_date'></td>
                <tr><td colspan="6"></td></tr>
                <tr><td><td></td><td><input type='submit' name='add_card' class="btn btn-primary btn-block"></td><td colspan="3"></td></tr>
            </form>
    </table>
            ...
            <div class="col-md-4">
             ...
             if($search_round_trip=="check"&&intval($search_children)>0){   
                ...
                $_SESSION["total_amount"]=(intval($leg_go['price'])*intval($search_adult)
                    +intval($leg_go['price'])*0.5*intval($search_children)
                    +intval($leg_back['price'])*intval($search_adult)
                    +intval($leg_back['price'])*0.5*intval($search_children));
                    }
                else if($search_round_trip=="check"&&intval($search_children)==0){
                $_SESSION["total_amount"]=(intval(($leg_go['price'])*($search_adult))+intval(($leg_back['price'])*($search_adult))); 
                }
                else if($search_round_trip!="check"&&intval($search_children)>0){
               $_SESSION["total_amount"]=intval($leg_go['price'])*($search_adult+($search_children*0.5)); 
                }
                else{
                
                $_SESSION["total_amount"]=intval($leg_go['price'])*($search_adult+($search_children)); 
                }
